...
// the logistic is introduced in the report
// using the periodicity of sine function and the relationship between sine and cosine
void ec_taylor(double* sinx, const double* x){
  int check[4];
  for (int i = 0; i < 4; i++) {
    double x1  = x[i];
    check[i] = 1;
    x1 = x1 - 2.0 * M_PI * floor((M_PI / 4.0 + x1)/(2.0 * M_PI));
    if (x1 >= M_PI * 3/4) {
      x1 = x1 - M_PI;
      check[i] = -1;
    }
...
int main() {
  ...
  double* x_ec = (double*) aligned_malloc(N*sizeof(double));
  double* sinx_ref = (double*) aligned_malloc(N*sizeof(double));
  double* sinx_ec_ref = (double*) aligned_malloc(N*sizeof(double));
  double* sinx_taylor = (double*) aligned_malloc(N*sizeof(double));
  double* sinx_ec_taylor = (double*) aligned_malloc(N*sizeof(double));
  ...
  for (long i = 0; i < N; i++) {
    ...
    x_ec[i] = (drand48()) * M_PI; // [0,pi], different x interval (need to be shifted)
    sinx_ref[i] = 0;
    sinx_ec_ref[i] = 0; // reference
    sinx_taylor[i] = 0;
    sinx_ec_taylor[i] = 0; // EC result
    sinx_intrin[i] = 0;
    sinx_vector[i] = 0;
  }
  ...
  for (long rep = 0; rep < 1000; rep++) {
    for (long i = 0; i < N; i+=4) {
      sin4_reference(sinx_ec_ref+i, x_ec+i);
    }
  }
  printf("EC Reference time: %6.4f\n", tt.toc());
  ...
  for (long rep = 0; rep < 1000; rep++) {
    for (long i = 0; i < N; i+=4) {
      ec_taylor(sinx_ec_taylor+i, x_ec+i);
    }
  }
  printf("EC Taylor time: %6.4f     Error: %e\n", tt.toc(), err(sinx_ec_ref, sinx_ec_taylor, N));

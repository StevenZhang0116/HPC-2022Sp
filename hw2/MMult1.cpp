...
// Blocking version
// void MMult1(long m, long n, long k, double *a, double *b, double *c) {
//   for(int j1=0;j1<m;j1+=BLOCK_SIZE){
//     for(int p1=0;p1<k;p1+=BLOCK_SIZE){
//       for(int i1=0;i1<n;i1+=BLOCK_SIZE){
//         for(int j=j1;j<j1+BLOCK_SIZE;j++){
//           for(int p=p1;p<p1+BLOCK_SIZE;p++){
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    double bandwidth = NREPEATS*(2*m*n+2*m*n*k)*sizeof(double)/1e9/time; 
